...
- Показ результата выполнения команды в системных уведомлениях (по умолчанию: включено). Уведомления предназначены для текстового вывода; длинные и бинарные выводы будут усечены или некорректны.
...
### Файлы конфигурации
Список команд: ~/.config/gucli/commands.toml
Логи выполнения и ошибок: ~/.config/gucli/gucli.log
Пример commands.toml по умолчанию:
```toml
...
Возможные проблемы:
- Программа создана в Tauri, которая уже включает основные зависимости - libwebkit2gtk-4.1-0 и libgtk-3-0, а также libappindicator3-1.
...
Ограничения в программе:
  - Команды выполняются с ограничением времени в 500 мс. Используйте только быстрые команды, возвращающие результат мгновенно. 
    При необходимости вы можете обойти это ограничение - добавить `&` в конце команды - так будет снята блокировка по времени.
  - Длина текста в уведомлении ограничена 200 символами - длинные сообщения могут вызвать зависание gtk.
